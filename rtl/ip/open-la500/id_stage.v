...
`ifdef HAS_LACC
// LACC 自定义指令前缀位于高位，后续字段携带 command、rk、rj 和 rd。
// lacc_req 表示当前指令属于 LACC；lacc_valid 检查 command 是否在有效范围。
...
// ds_to_es_bus 是“译码级 Decode Stage → 执行级 Execute Stage”的流水线总线。
// 它把当前指令译码得到的控制信息、立即数、寄存器值和 LACC command
// 一次性打包，在 ds_allowin/es_allowin 握手成功时送入执行级寄存器。
assign ds_to_es_bus = {
                       `ifdef HAS_LACC
...
`ifdef HAS_LACC
    // 当前约定 inst[31:28]==4'b1100 时为 LACC 自定义指令。
