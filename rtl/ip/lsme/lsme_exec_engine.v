...
// 直接复用同一个 Z/P/ZA 与 MOPA 核。Softmax、向量加法和流式 RMSNorm
// 使用专用整数逻辑；其中 RMSNorm 复用单字访存端口和共享迭代算术单元。
module lsme_exec_engine (
    ...
    output reg [31:0]  perf_rmsnorm_rows,
    ...
    localparam [5:0] ST_RN_WAIT       = 6'd33;
    ...
    reg [4:0] out_shift;
    reg [4:0] score_shift;
    reg [7:0] norm_token_frac;
    reg [7:0] norm_gain_frac;
    reg [7:0] head_count;
    reg [7:0] head_dim;
    ...
    wire rmsnorm_busy;
    wire rmsnorm_done;
    wire [7:0] rmsnorm_error;
    wire rmsnorm_mem_req_valid;
    wire rmsnorm_mem_req_write;
    wire [31:0] rmsnorm_mem_req_addr;
    wire [31:0] rmsnorm_mem_req_wdata;
    wire [3:0] rmsnorm_mem_req_wstrb;
    wire [31:0] rmsnorm_rows_completed;
    wire [31:0] rmsnorm_memory_words;
    wire rmsnorm_compute_active;
    wire rmsnorm_memory_stall;
    ...
    lsme_rmsnorm_core u_rmsnorm (
        .clk(clk), .reset(reset), .start(rmsnorm_start),
        .busy(rmsnorm_busy), .done(rmsnorm_done),
        .error_code(rmsnorm_error),
        .src_addr(src0_addr), .gain_addr(src1_addr), .dst_addr(dst_addr),
        .rows(m_dim), .columns(n_dim), .batch_count(batch_dim),
        .src_row_stride(src0_row_stride),
        .dst_row_stride(dst_row_stride),
        .src_batch_stride(src0_batch_stride),
        .dst_batch_stride(dst_batch_stride),
        .token_frac(norm_token_frac), .gain_frac(norm_gain_frac),
        .mem_req_valid(rmsnorm_mem_req_valid),
        .mem_req_ready(mem_req_ready && state == ST_RN_WAIT),
        .mem_req_write(rmsnorm_mem_req_write),
        .mem_req_addr(rmsnorm_mem_req_addr),
        .mem_req_wdata(rmsnorm_mem_req_wdata),
        .mem_req_wstrb(rmsnorm_mem_req_wstrb),
        .mem_rsp_valid(mem_rsp_valid && state == ST_RN_WAIT),
        .mem_rsp_rdata(mem_rsp_rdata), .mem_rsp_error(mem_rsp_error),
        .rows_completed(rmsnorm_rows_completed),
        .memory_words(rmsnorm_memory_words),
        .compute_active(rmsnorm_compute_active),
        .memory_stall(rmsnorm_memory_stall)
    );
    ...
    always @(posedge clk) begin
        ...
        if (reset) begin
            ...
            if (v2_compute_active || softmax_busy || rmsnorm_compute_active)
                perf_compute_cycles <= perf_compute_cycles + 32'd1;
            if (v2_memory_stall || rmsnorm_memory_stall)
            ...
            case (state)
                ...
                ST_DECODE: begin
                    ...
                    norm_token_frac <= descriptor[13][7:0];
                    norm_gain_frac <= descriptor[13][15:8];
                    ...
                    if (descriptor[5][15:0] == 0 ||
                        ...
                        case (descriptor[0][7:0])
                            ...
                            `LSME_OP_RMSNORM: begin
                                if (descriptor[5][31:16] > 64 ||
                                    descriptor[13][7:0] > 15 ||
                                    descriptor[13][15:8] > 15) begin
                                    error_code <= ERR_BAD_DIM;
                                    state <= ST_ERROR;
                                end
                                else begin
