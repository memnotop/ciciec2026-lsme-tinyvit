...
module lsme_core #(
    ...
    lsme_mopa_core #(.MOPA_LANES(MOPA_LANES)) u_mopa (
        .clk(clk),
        .reset(reset),
        .start(mopa_start),
                    ...
                    if (macro_start) begin
                        ...
                        case (req_command)
                            CMD_CTRL: begin
                                rsp_valid <= 1'b1;
                                case (req_imm)
                                    // LACC 查询与 AXI CSR 使用同一个能力字格式，
                                    // 防止裸机演示中两条控制路径报告不同的功能集合。
                                    // bit5=RMSNorm；0xbf 还表明 macro8、burst、
                                    // scratchpad、Softmax、VADD 等 V2 功能已经启用。
