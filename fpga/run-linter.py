...
def usage():
    print("Usage: python3 run-linter.py <path-to-vivado-xpr>", file=sys.stderr)
...
def transcoding(src: Path, target: Path):
    with src.open("rb") as fd:
        ...
        detected = res['encoding'] if res['confidence'] > 0.5 else None
        encodings = ["utf-8", "gb18030"]
        if detected and detected.lower() not in ("utf-8", "gb18030"):
        ...
        for encoding in encodings:
            ...
            s = rawdata.decode("utf-8", "replace")
