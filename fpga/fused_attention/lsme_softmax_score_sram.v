...
// 面向融合 Attention score SRAM 的逐元素整数 Softmax。
// 使用 32767 * 2^(-delta/32) 近似指数，其中
// delta = clamp((row_max - score) >> score_shift, 0, 255)。
// 与通用 lsme_softmax_core 使用完全相同的 max/exp/reciprocal/rounding
// 算法。不同点是 score_data 由外部 SRAM 按 score_index 提供，因此无需
// 2048 位 row_in 总线及其动态位选网络。
module lsme_softmax_score_sram (
    input               clk,
    input               reset,
    input               start,
    input      [6:0]    count,
    input      [4:0]    score_shift,
    input signed [31:0] score_data,
    output reg          busy,
    output reg          done,
    output reg          stream_valid,
    output reg [5:0]    score_index,
    ...
    always @(*) begin
        delta_full = $signed({row_max[31], row_max}) - $signed(score_data);
        if (delta_full <= 0)
            delta_scaled = 0;
        else
        ...
        norm_product = mul_u16_lut(exp_buf[score_index], reciprocal);
            ...
            case (state)
                ST_IDLE: begin
                    ...
                    if (start) begin
                        ...
                        score_index <= 6'd0;
                        row_max <= $signed(score_data);
                        state <= ST_MAX;
                    end
                end

                ST_MAX: begin
                    if ($signed(score_data) > row_max)
                        row_max <= $signed(score_data);
                    if ({1'b0, score_index} == elem_count-7'd1) begin
                        score_index <= 6'd0;
                        exp_sum <= 0;
                        state <= ST_EXP;
                    end
